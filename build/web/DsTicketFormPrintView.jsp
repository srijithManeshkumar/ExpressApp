 ...
 */
/** 
 * MODULE:	DsTicketFormPrintView.jsp
 * 
 * DESCRIPTION: 
 * 
...
catch (Exception e)
{
	...
	Log.write(Log.ERROR, "Trapped in DsTicketFormPrintView.jsp");
}
%>

<jsp:useBean id="ticketBean" scope="request" class="com.alltel.lsr.common.objects.DsTicketBean" />
...
<%
Log.write(Log.DEBUG_VERBOSE, "DsTicketFormPrintView.jsp --- ");
...
String m_strTcktHdrQry = "SELECT STTS_CD FROM DSTICKET_T WHERE TCKT_SQNC_NMBR = " + m_iTcktSqncNmbr;
...
<table align=left border=0 cellspacing=1 cellpadding=0>
	...
	<tr>
		...
		<td align=left><%=rsTcktHdr.getString("STTS_CD")%></td>
	</tr>
</table>
<BR CLEAR=ALL>
<HR>
...
<%
Log.write(Log.DEBUG_VERBOSE, "DsTicketFormView.jsp --- m_strTcktFormType = " + m_strTcktFormType);

FormSection fs;
FormField ff;
boolean bMoreForms = true;

String m_strPrintQry = "SELECT DISTINCT FRM_SQNC_NMBR FROM DSTICKET_T, SERVICE_TYPE_FORM_T WHERE TCKT_SQNC_NMBR = " +
	m_iTcktSqncNmbr + " AND SERVICE_TYPE_FORM_T.SRVC_TYP_CD = 'T' AND SERVICE_TYPE_FORM_T.TYP_IND = 'S'";
...
%>
	...
	<%
		...
		else
		{
		  ...
		  {
			m_strSrcFldQry = "SELECT " + ff.getSrcDbClmnNm() + " FROM " + ff.getSrcDbTblNm() + " WHERE TCKT_SQNC_NMBR = " + m_iTcktSqncNmbr + " AND VRSN = " + m_iCurrentTcktVrsn;
		  ...
		  {
			m_strSrcFldQry = "SELECT " + ff.getSrcDbClmnNm() + " FROM " + ff.getSrcDbTblNm() + " WHERE TCKT_SQNC_NMBR = " + m_iTcktSqncNmbr + " AND VRSN = " + m_iCurrentTcktVrsn + " AND FRM_SCTN_OCC = " + iSvFrmSctnOcc;
