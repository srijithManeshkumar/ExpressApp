...
<FORM NAME="WNPDateMonthReportView" METHOD=POST ACTION="<%=strPost%>">
...
try {
	...
	String strOCNQuery = "";
	if (strPost != null && strPost.equals("WNPSummaryReport2.jsp"))
	{	//weed em out if WNP report
		strOCNQuery = "SELECT DISTINCT O.OCN_CD, O.OCN_NM FROM OCN_T O, COMPANY_T C " +
			" WHERE O.CMPNY_SQNC_NMBR=C.CMPNY_SQNC_NMBR AND C.CMPNY_TYP in ('R','C') AND C.CMPNY_NM NOT LIKE 'TSI T%' ORDER BY O.OCN_CD";
	}
	else
	{
		strOCNQuery = "SELECT DISTINCT O.OCN_CD, O.OCN_NM FROM OCN_T O, COMPANY_T C " +
