<%-- 
    Document   : BDPActivityExcelReport
    Created on : May 24, 2011, 4:44:43 PM
    Author     : satish.t
--%>
...
<%
            ...
            response.setHeader("Content-Disposition", "attachment; filename=" + "BDPActivityReport" + ".xls");
...
%>
		<jsp:forward page="BDPActivityReportForm.jsp"/>;
<%
		return;
	}
...
<%
	...
	BDPSLAReportByUserBean Report = new BDPSLAReportByUserBean();
	...
	Report.setUserids( strUserids );
	String[] strEmployeeGroups = alltelRequest.getAttributeValue("groupids");
	boolean egFlag = false;
	if(  isElementOf( strProductType, "none"  )) {
	alltelRequest.getHttpRequest().setAttribute("slastat", "You must select a Product Type for this Report!");
	%>
		<jsp:forward page="BDPActivityReportForm.jsp"/>;
	<%
		return;
	...
	Report.extractEmployeeGroups( strEmployeeGroups, egFlag );
