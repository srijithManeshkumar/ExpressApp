...
<% 
	...
	alltelResponse.getHttpResponse().setHeader("Content-Disposition", ("attachment; filename = KPEN_BDP_COMPREPORT_" + getCurrentDate_Simpe() + ".xls" ));
...
<%
  // user prepared statement later. This query runs once a month...    
	...
	try{
		String strQry = "select  d.dwo_sqnc_nmbr as OrderNumber, h.DWO_STTS_CD_IN as OrderStatus, to_char(h.DWO_HSTRY_DT_IN, 'mm-dd-yyyy HH24:MI:SS Am') as  statusdate, "
			...
			+ 	"and P.PRDCT_TYP_CD = D.PRDCT_TYP_CD and h.DWO_STTS_CD_IN = 'COMPLETED' "
			+ 	"and h.DWO_HSTRY_DT_IN BETWEEN to_date('" + strStartDate + "','YYYYMMDD') "
			+ 	"AND to_date('" + strEndDate + "','YYYYMMDD') and s.typ_ind = 'W' "
			+ 	"and a.TYP_IND = 'W' "
			+ 	"	union "
			+ 	"select distinct d.dwo_sqnc_nmbr as OrderNumber, h.DWO_STTS_CD_IN as OrderStatus, to_char(h.DWO_HSTRY_DT_IN, 'mm-dd-yyyy HH24:MI:SS Am') as  statusdate, "
			...
			+ 	"and P.PRDCT_TYP_CD = D.PRDCT_TYP_CD and h.DWO_STTS_CD_IN ='SERVICE COMPLETE' "
			+ 	"and h.DWO_HSTRY_DT_IN BETWEEN to_date('" + strStartDate + "','YYYYMMDD') "
			+ 	"AND to_date('" + strEndDate + "','YYYYMMDD') and s.typ_ind = 'X' "
			//libby, 11/13/2009, RIS 1077681, added types 'C','W','Y','Z'
			//libby, 11/13/2009, moved types 'P', 'E' from 'DSTAC IPTST COMP' to this query 'SERVICE COMPLETE'   
			+ 	"and p.TYP_IND = 'X' AND  D.PRDCT_TYP_CD IN ( 'A', 'D', 'S', 'T', 'P', 'E', 'C', 'W', 'Y', 'Z') "
			+ 	" union "
			+ 	"select distinct d.dwo_sqnc_nmbr as OrderNumber, "
			+ 	"h.DWO_STTS_CD_IN as OrderStatus, to_char(h.DWO_HSTRY_DT_IN, 'mm-dd-yyyy HH24:MI:SS Am') as  statusdate, h.mdfd_userid as UserId, "
			...
			+ 	"and h.DWO_STTS_CD_IN ='DSTAC IPTST COMP' "
			+ 	"and h.DWO_HSTRY_DT_IN BETWEEN to_date('" + strStartDate + "','YYYYMMDD') "
			+ 	"AND to_date('" + strEndDate + "','YYYYMMDD') "
			+ 	"and s.typ_ind = 'X' and p.TYP_IND = 'X' "
			//gao, 08/25/2006, RIS 1449, add type 'L'
			//libby, 11/13/2009, RIS 1077681, add types 'X'
			//libby, 12/03/2009, RIS 348799, add type 'W' EIA work queue changed to DSTAC IPTST COMP
			//libby, 11/30/2010 add type 'B'  
			+ 	"and a.TYP_IND(+) = 'X' AND  D.PRDCT_TYP_CD IN ( 'M', 'I', 'L', 'X', 'W', 'B')"
			+ 	"order by   Client, PRODUCTName, UserId ";     
