 ...
 *	05/01/2003 psedlak put in generic locking
 */
...
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- ");
...
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- m_strSqncNmbr = " + m_strSqncNmbr);
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- m_strVrsn = " + m_strVrsn);
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- m_strFrmSqncNmbr = " + m_strFrmSqncNmbr);
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- m_strForm = " + m_strForm);
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- m_strFormType = " + m_strFormType);
...
int m_iVrsn = 0;
final String m_strTypInd = "T";

// Get fields from TICKET_T that will show up on the top of each Ticket related page.
final String m_strHdrQry = "SELECT A.TCKT_STTS_CD, A.TELNO, NVL(L.USERID, '_NOT_LOCKED_'), " +
	" TO_CHAR(NVL(L.LCK_DT, sysdate), 'MM/DD/YYYY @ HH24:MI:SS'), S.DTL_ORDR_HSTRY_IND " +
	" FROM TICKET_T A, LOCK_T L, STATUS_T S " +
	" WHERE A.TCKT_SQNC_NMBR = ? AND A.TCKT_STTS_CD = S.STTS_CD AND S.TYP_IND='" +  m_strTypInd + "' " +
...
ResultSet rsNote = stmtHdr.executeQuery("SELECT COUNT(*) FROM TICKET_NOTES_T WHERE TCKT_SQNC_NMBR="+ m_iSqncNmbr);
rsNote.next();
...
String strSrvcTypCd = "2";
String strSttsCd = rsHdr.getString("TCKT_STTS_CD");
...
Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- strLockedUserID = " + strLockedUserID);
...
<%		if (sdm.isAuthorized("TCKT_NOTES_VIEW") ) {
%>
			<td align=center
				<%if (strNotes != null) 
...
<% 
		...
		if ( bOrderLocked ) 
		{
			Log.write(Log.DEBUG_VERBOSE, "i_TicketHeader.jsp --- Tweaked security tag since its locked");
					...
					if (!bAlreadyLocked)
					{
						LockBean lockedObj = new LockBean(m_strTypInd, m_iSqncNmbr);
                                                if (bSelfLocked)        //We have it, so just update existing lock
                                                {
                                                        ...
                                                        {
                                                            //lock failed!
                                                            Log.write(Log.ERROR, "i_TicketHeader.jsp --- Error updating locked seq=" + m_iSqncNmbr);
...
<%                                                          break;
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
							//lock failed!
							Log.write(Log.ERROR, "i_TicketHeader.jsp --- Error locking seq=" + m_iSqncNmbr);
